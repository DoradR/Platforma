...
import { 
    ...
    PRODUCT_VIDEO_UPLOAD_REQUEST,
    PRODUCT_VIDEO_UPLOAD_SUCCESS,
    PRODUCT_VIDEO_UPLOAD_FAIL,
...
export const uploadProductVideo = (videoFile, videoName, productId) => async (dispatch, getState) => {
    try {
        const formData = new FormData()
        formData.append("video_course", videoFile)
        formData.append("video_name", videoName)
        formData.append("product_id", productId)

        dispatch({ type: PRODUCT_VIDEO_UPLOAD_REQUEST })
        ...
        const { data } = await axios.post(
            `${backendUrl}/api/products/upload-video/`, 
            formData, 
            configurations)

        dispatch({
            type: PRODUCT_VIDEO_UPLOAD_SUCCESS,
            payload: data,
        })

    } catch (error) {
        dispatch({
            type: PRODUCT_VIDEO_UPLOAD_FAIL,
            payload: error.response && error.response.data.detail
                ? error.response.data.detail
                : error.message,
        })
