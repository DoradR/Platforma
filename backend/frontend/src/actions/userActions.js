...
import { 
    ...
    COURSE_TOKEN_REQUEST,
    COURSE_TOKEN_SUCCESS,
    COURSE_TOKEN_FAIL,
...
export const getCourseToken = (courseId) => async (dispatch, getState) => {
    try {
      dispatch({ type: COURSE_TOKEN_REQUEST })
      ...
      const { data } = await axios.get(
        `${backendUrl}/api/users/course/${courseId}/token/`, 
        configurations
      )
  
      dispatch({
        type: COURSE_TOKEN_SUCCESS,
        payload: data,
      });
  
    } catch (error) {
      dispatch({
        type: COURSE_TOKEN_FAIL,
        payload: error.response && error.response.data.detail
          ? error.response.data.detail
          : error.message,
      });
